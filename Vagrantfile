...
Vagrant.configure(2) do |config|
  ...
  config.vm.provider 'virtualbox' do |vb|
    ...
    vb.name = options['domain']
  end

  # machine name (for vagrant console)
  config.vm.define options['domain']

  # machine name (for guest machine console)
  config.vm.hostname = options['domain']

  # network settings
  config.vm.network 'private_network', ip: options['ip']
  config.vm.network "forwarded_port", guest: 5432, host: 54321, id: "pgsql"

  # sync: project folder (host machine) -> folder '/app' (guest machine)
  ...
  config.vm.post_up_message = "Application URL: http://#{options['domain']}"
end
